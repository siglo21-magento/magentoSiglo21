...
namespace Aventi\SAP\Rewrite\Aheadworks\Ca\Controller\User;
...
class SavePost extends \Aheadworks\Ca\Controller\User\SavePost
{
    ...
    {
        parent::__construct($context, $customerSession, $customerRepository, $customerExtractor, $dataObjectHelper, $companyUserManagement, $dataPersistor, $extensionAttributesManagement, $companyRepository, $authorizationManagement);
    ...
    private function performSave(CustomerInterface $companyUser, array $data)
    {
        ...
        $rootUser = $this->companyUserManagement->getRootUserForCompany($this->getCurrentCompanyId());

        if($rootUser->getId() != $companyUser->getId()) {

            $companyUser->setCustomAttribute('sap_customer_id', $rootUser->getCustomAttribute('sap_customer_id')->getValue());
            $companyUser->setCustomAttribute('slp_code', $rootUser->getCustomAttribute('slp_code')->getValue());
            $companyUser->setCustomAttribute('identification_customer', $rootUser->getCustomAttribute('identification_customer')->getValue());
            $companyUser->setCustomAttribute('owner_code', $rootUser->getCustomAttribute('owner_code')->getValue());
            $companyUser->setCustomAttribute('user_code', $rootUser->getCustomAttribute('user_code')->getValue());
            $companyUser->setCustomAttribute('seller_email', $rootUser->getCustomAttribute('seller_email')->getValue());

	...
	*/
	...
	public function generateCssByStore($storeId){
		...
		if($lazyLoad && $lazyLoadImg != ""){
			$html .= '.parent_lazy:not(.lazy_loaded), .intro-slider .page-header-sliders.loading:before, .mgs-product-tab .data.item.title + .content.div-loading::after { background-image: url("'. $this->getMediaUrl() . 'mgs/setting/' . $lazyLoadImg.'") !important; }';
		}
							...
							$html .= '<div class="'.$blockClass.'">';
								$html .= '<div class="line">';
									...
									foreach($blocks as $_block){
										...
										$html .= '<div class="panel-block-row ' . $this->getChildClass($_block, $setting) . '" style="z-index: 999;">';

						...
						case 'copyblock':
							if($('.btn-dulicate').length==0){
								$('.btn-addnewblock').each(function(){
									...
									$(this).parent().append('<button class="action btn btn-default btn-dulicate" type="button" onclick="pasteBlock('+pageId+', \''+blockName+'\')"><em class="fa fa-plus-circle"></em>&nbsp;Paste Copied Block</button>');
								});
							}
...
function pasteBlock(pageId, blockName){
	var url = WEB_URL+'fbuilder/index/pasteblock/page_id/'+pageId+'/block_name/'+blockName;
	setLocation(url);
}

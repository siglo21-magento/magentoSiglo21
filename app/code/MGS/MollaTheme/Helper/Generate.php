...
class Generate extends \MGS\Fbuilder\Helper\Generate
{
	...
	public function generateCssCustomWidth($customWidth){
		...
		return '.page.messages .messages { max-width: '. $customWidth .'px; } @media(min-width: '. $customWidthPadding .'px){ body.custom .frame, body.custom .breadcrumbs .items , body.custom .page-main > .columns, .breadcrumbs .items { max-width: '. $customWidth .'px;} }';
	}
}
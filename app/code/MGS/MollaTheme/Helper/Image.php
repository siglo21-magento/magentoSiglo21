			...
			case 'category_product_virtual':
				...
				if($request->getParam('dimention')){
					...
					if($category && $category->getImageDimentionList()){
						...
						$dimention = $this->getStoreConfig("themesettings/product_image_dimention/category_product_virtual");
					}
				}
				
				$dimention = $this->scopeConfig->getValue("themesettings/product_image_dimention/category_product_virtual",\Magento\Store\Model\ScopeInterface::SCOPE_STORE);
				$arrDimention = explode('x',$dimention);
				if(($dimention!='') && (count($arrDimention)>0)){
					$width = trim($arrDimention[0]);
					if(isset($arrDimention[1])){
